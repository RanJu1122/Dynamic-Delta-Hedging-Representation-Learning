...
> 最新网格：默认观察、模型、precompute 和初始持仓统一为 full63（7 期限 × 9 level，包含 1.2）。此前 56 节点计时属于历史验证。新命令见 [统一 full63 说明](UNIFIED_FULL63_CN.md)。

> 2026-09-14 跨期限共享更新：`precompute` 现在按日期批量计算尚未缓存的期限。同一固定 Alpha、同一 Spot bump 只推进到这些期限的最远到期日，在各到期日保存股价快照。各期限仍使用原来的收益、控制变量、IV 反解、Beta/Delta/标准误及自身网格前缀的修补统计。随机数种子、抽取顺序和路径推进算式保持不变。引擎指纹已更新，旧库会明确报 `engine mismatch`，请用新的输出目录重建；不要手改旧库指纹。输出仍按日期/期限分片，新版本生成的分片可断点续算。一个日期的批量计算若在落盘前中断，需要重算该日期尚未保存的期限。

本次验证：完整测试 125 项通过；最终针对预计算的 13 项测试通过。使用修改前保存的 `montecarlo.py` 和 `hedging.py`，与新批量入口比较两天真实曲面。参数均为 40k 路径、801 空间节点、每日 2 个子步、7 个期限、8 个执行价比例、5 个 Alpha。每一天的 280 行、49 列全部通过 `assert_frame_equal(check_exact=True)`，包括 Beta、Delta、标准误、反解选择和质量标记。

| 日期 | 旧逐期限计算 | 新共享计算 | 本次加速比 |
|---|---:|---:|---:|
| 2026-01-14 | 75.60 秒 | 28.21 秒 | 2.68 倍 |
| 2026-03-31 | 77.85 秒 | 28.85 秒 | 2.70 倍 |

以上为本机两日抽样，计时不含数据加载、库写盘和绘图，并非全历史耗时保证；没有运行全历史映射或正式回测。保留零 Spot 变化的原数据未修改。验证结果、输出和旧代码快照见 `output/mc_shared_precompute_20260914/`，可在项目根目录用 `PYTHONPATH=. .venv/bin/python output/mc_shared_precompute_20260914/compare.py` 复现。当前仍按原顺序生成随机数矩阵，优化的是期限之间的重复网格构建和路径推进；没有把本次改动扩展为随机数流式存储改造。
...
> 2026-09-11 补充：用户已恢复 2026-03-31、04-01、04-02 三个快照，spot 保持原值。
> 本文原有“剔除三天”的描述属于历史口径；当前上游需重建，旧 56 节点库可复制后补算 21 个分片。
> 当前命令见 [到期续开与恢复数据](STEP7_RENEWAL_CN.md)。
...
预计算不读取 Beta 标签、因子、预测器或未来价格。每个分片是一个 `日期 × tenor`，保存全部预设 `level × alpha` 节点的 MC 结果。同一日期、Alpha 和 bump 下，不同期限共享路径前缀，不同 strike 共享对应期限的终点股价；不同预测策略不会增加预计算次数。不同 Alpha 和上下 bump 仍分别推进股价，共用随机数。
...
| levels，K/refSpot | `0.4, 0.5, …, 1.2`，共 9 档 |
| Alpha | `0, 0.5, 1, 1.5, 2` |
| 路径数 | `40000` |
| 随机种子 | `20260807` |
| 对偶采样 | 开启；路径数包含全部正负路径 |
...
默认定价轴与当前 full book 的 63 个初始单元一致，包含 level=1.2，不包含 1M。期限超出当天报价范围会记录为 unsupported，不外推。
